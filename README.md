...
**How to test and evidence the human oversight control in an AI system.** Audit-format templates for showing that human-in-the-loop review works, for EU AI Act Article 26(2) and for internal control testing generally.

Governance frameworks and the EU AI Act both assign consequential decisions to human reviewers. Neither specifies how an organisation would know those reviewers can still detect the errors they are there to catch. This repository holds preliminary, non-validated record formats for the components that can be assessed, and says which cannot.
...
Some of what follows may transfer, though none of it has been tried there. Capabilities stated as observable performance and assessed against a criterion set in advance (record 01); the conditions of volume and time pressure that determine what any such assessment means (record 02); whether the work can still be done without AI assistance (record 04). Whether these are the right shape for an evaluator population — small, highly selected, and working with no established baseline — is untested. Record 03 may not transfer at all: seeding an error needs a known correct answer, and judging whether a novel argument is sound has none.
